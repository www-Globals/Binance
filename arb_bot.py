...
# We'll store prices as simple floats (last price from ticker)
ws_prices = {}
symbol_info = {}
...
# Build list of all symbols needed
...
def update_prices():
    """Fetch all symbol prices via REST every 2 seconds."""
    while True:
        try:
...
def get_price(symbol):
    """Return latest price (fallback to REST if not in cache)."""
...
def calc_profit(a,b,c,amount):
    try:
        ...
        p_ab = get_price(sym1)   # we use mid price (approx)
        p_bc = get_price(sym2)
        p_ca = get_price(sym3)
...
if __name__ == "__main__":
    ...
    # Start REST price updater instead of WebSocket
