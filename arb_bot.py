...
# ---------- PRODUCTION RULES (EXTREMELY LOW FOR TESTING) ----------
KES_RATE = 130
TAKER_FEE = 0.001
PROFIT_THRESHOLD = 0.0001     # 0.01% – almost any profit
...
MIN_TRADE_USDT = 5            # small amount
MAX_SPREAD = 0.02             # 2% spread allowed
MIN_PROFIT_KES = 1
...
def execute_trade(a, b, c, trade_amount_usd):
    ...
    try:
        ...
        # Always log the attempt, even if failed
        log_to_supabase({
            "pair": pair,
            "profit_kes": loss_kes,
            "status": "LOSS",
...
def scan():
    ...
    if not first_scan_done and trades_today == 0:
        ...
        test_amount = min(trade_amount, 10.0)  # use small amount
...
if __name__ == "__main__":
    ...
    # Insert a startup log to confirm Supabase connection
