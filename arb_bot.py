...
MANUAL_PROXY = os.getenv("PROXY_URL")   # optional – if set, use this proxy
...
def get_working_proxy():
    """Fetch a list of free proxies and return the first one that works with Binance testnet."""
    if MANUAL_PROXY:
        print(f"[PROXY] Using manual proxy: {MANUAL_PROXY}")
        return MANUAL_PROXY

    print("[PROXY] Fetching free proxies from ProxyScrape...")
    try:
        # free proxy API – returns a list of HTTP proxies
        ...
        print(f"[PROXY] Found {len(proxies)} proxies. Testing...")
    except Exception as e:
        print(f"[PROXY] Failed to fetch proxy list: {e}. Using fallback hardcoded list.")
        proxies = [
            "45.79.202.80:3128",
            "51.75.126.153:3128",
            "138.68.1.144:8080",
    ...
    for proxy in proxies:
        ...
        try:
            # Quick test: try to fetch exchange info with this proxy
            test_ex = ccxt.binance({
                'enableRateLimit': True,
                'proxy': proxy_url,
            })
            test_ex.set_sandbox_mode(True)
            test_ex.load_markets()
            print(f"[PROXY] ✅ Proxy {proxy} works!")
            return proxy_url
        except Exception:
            print(f"[PROXY] ❌ Proxy {proxy} failed.")
            continue

    print("[PROXY] ⚠️ No working proxy found. Will proceed without proxy (likely to fail).")
    return None

# ---------- INITIALISE CCXT EXCHANGE ----------
...
if proxy_url:
    ...
    print(f"[CLIENT] Using proxy: {proxy_url}")
else:
    print("[CLIENT] No proxy set – direct connection (may fail)")

# Test connection
try:
    exchange.load_markets()
    print("[CLIENT] ✅ Successfully connected to Binance testnet.")
except Exception as e:
...
# ---------- PRODUCTION RULES (LOW FOR TESTING) ----------
KES_RATE = 130
TAKER_FEE = 0.001
...
if __name__ == "__main__":
    ...
    time.sleep(5)
    telegram("🚀 <b>BOT STARTED (TESTNET – CCXT with proxy auto‑discovery)</b>")
