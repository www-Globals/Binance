...
PROXY_URL = os.getenv("PROXY_URL")   # optional manual proxy
...
def get_working_proxy():
    if PROXY_URL:
        print(f"[PROXY] Using manual proxy: {PROXY_URL}")
        return PROXY_URL
    ...
    sources = [
        ...
        "https://proxylist.geonode.com/api/proxy-list?protocol=http&limit=50&country=NL,DE,FR,GB,SG",
    ]
    proxies = []
    for src in sources:
        try:
            ...
            if resp.status_code == 200:
                ...
                proxies.extend([p.strip() for p in lines if p.strip()])
        except:
            continue
    proxies = list(set(proxies))
    print(f"[PROXY] Found {len(proxies)} candidates. Testing...")

    for proxy in proxies:
        proxy_url = f"http://{proxy}"
        try:
            test_ex = ccxt.binance({'enableRateLimit': True, 'proxy': proxy_url})
    ...
    print("[PROXY] No working proxy found. Proceeding without proxy (will likely fail).")
    return None
...
if proxy_url:
    ...
    print(f"[CLIENT] Proxy set: {proxy_url}")
...
if __name__ == "__main__":
    ...
    time.sleep(5)
    telegram("🚀 <b>BOT STARTED (TESTNET – CCXT with proxy)</b>")
