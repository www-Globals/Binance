...
# 🔽 LOWERED THRESHOLDS FOR TESTNET
PROFIT_THRESHOLD = 0.001        # 0.1%
SLIPPAGE_BUFFER = 0.001
MIN_PROFIT_KES = 5              # 5 KES
MAX_SPREAD = 0.005              # 0.5%

# 🛡️ RISK LIMITS (unchanged)
...
def scan():
    ...
    # --- SCAN LOGGING ---
    scan_count += 1
    if scan_count % 10 == 0:   # print every 10 scans (about every 10 seconds)
        print(f"[SCAN #{scan_count}] Best: {best_profit:.2f}% from {best_tri[0]}{best_tri[1]}{best_tri[2] if best_tri else 'None'}, spread {best_spread:.3%}, threshold {PROFIT_THRESHOLD:.2f}%")
    elif best_tri and best_profit >= PROFIT_THRESHOLD:
        # also print when an opportunity is found
        print(f"[SCAN #{scan_count}] OPPORTUNITY: {best_tri[0]}{best_tri[1]}{best_tri[2]} {best_profit:.2f}% >= {PROFIT_THRESHOLD:.2f}%")
    ...
    # else: no opportunity, but we already printed the best above
...
if __name__ == "__main__":
    load_markets()
    if VALID_TRIANGLES:
        initial_balance = get_total_usdt_balance()
        log_to_supabase({
            "timestamp": datetime.datetime.utcnow().isoformat(),
