...
def get_symbols_from_triangles():
    """Return candidate symbols in both formats."""
    candidates = set()
...
# Filter to only those that actually exist in exchange.markets
def filter_existing_symbols(candidates):
    if not exchange.markets:
        return candidates
    existing = [s for s in candidates if s in exchange.markets]
    if not existing:
        print("[WARN] None of the candidate symbols exist in exchange.markets.")
        print("[WARN] Available symbols sample:", list(exchange.markets.keys())[:20])
    return existing
...
def load_markets():
    ...
    try:
        ...
        if matched == 0:
            print("[WARN] No symbols matched – check available symbols in logs above.")
    except Exception as e:
        print(f"[FATAL] Could not load markets: {e}")

def round_qty(qty, symbol):
    # Try the symbol as-is; if not found, try the other format
...
def calc_profit(a, b, c, amount):
    try:
        # Try slash format first, then no-slash
        sym1 = f"{b}/{a}"
        sym2 = f"{b}/{c}"
        sym3 = f"{c}/{a}"
        ...
        if 0 in (p_ab, p_bc, p_ca):
            ...
            if 0 in (p_ab, p_bc, p_ca):
                return 0, 0, 0
            # Update symbols for the rest of the function
