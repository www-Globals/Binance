...
# ---------- INITIALISE BINANCE CLIENT (FIXED FOR TESTNET) ----------
if USE_TESTNET:
    # Force testnet API URL – this avoids geo‑blocking
    client = Client(BINANCE_API_KEY, BINANCE_SECRET, api_url='https://testnet.binance.vision/api')
else:
...
# ---------- GLOBALS ----------
ws_prices = {}       # symbol -> float (last price)
symbol_info = {}
...
def keep_alive():
    """Ping /health every 9 minutes to keep Render from sleeping."""
    while True:
        time.sleep(540)
        if RENDER_URL:
            try:
...
# ---------- REST PRICE UPDATER (replaces WebSocket) ----------
def update_prices():
    """Fetch all needed symbol prices every 2 seconds via REST."""
    while True:
        try:
            ...
            for ticker in tickers:
                ...
                if sym in ALL_SYMBOLS:
                    ws_prices[sym] = float(ticker['price'])
...
def log_to_supabase(data):
    """Insert a row into Supabase 'trades' table."""
...
def load_filters():
    """Load symbol filters (lot size, min notional) for rounding."""
    try:
...
def get_price(symbol):
    """Return latest price (cached or via REST)."""
    if symbol in ws_prices and ws_prices[symbol] > 0:
...
def get_spread(symbol):
    """Approximate spread from order book (REST)."""
    try:
...
def get_total_usdt_balance():
    """Fetch account balance in USDT equivalent."""
    try:
        ...
        for asset in account['balances']:
            ...
            if asset['asset'] == 'USDT':
                ...
                price = get_price(asset['asset'] + 'USDT')
                if price > 0:
                    total += free * price
...
def calc_profit(a, b, c, amount):
    """Calculate gross/net profit % for a triangle."""
    try:
        sym1 = f"{b}{a}"   # buy b with a (USDT)
        sym2 = f"{b}{c}"   # sell b for c
        sym3 = f"{c}{a}"   # sell c for a (USDT)
        ...
        # Simulate the trade: start with `amount` USDT
...
def execute_trade(a, b, c, trade_amount_usd):
    """Execute the triangle trade with automatic rollback on failure."""
    ...
    try:
        ...
        if qty_b * get_price(sym1) < symbol_info[sym1]['minNotional']:
        ...
        if qty_c * get_price(sym2) < symbol_info[sym2]['minNotional']:
        ...
        # --- LOSS ---
        consecutive_fails += 1
        loss_usd = trade_amount_usd * 0.003   # estimated max loss
        ...
        # ROLLBACK: reverse all executed legs
        for symbol, qty, side in reversed(executed):
            try:
                ...
                print(f"Rollback failed for {symbol}")
...
def scan():
    """Main scanning loop – called repeatedly."""
    reset_daily()
    if is_paused():
        return
...
if __name__ == "__main__":
    ...
    time.sleep(5)  # let things initialise
