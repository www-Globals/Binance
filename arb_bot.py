...
# Load markets once to see available symbols
try:
    exchange.load_markets()
    print("[CLIENT] ✅ Connected to Binance testnet.")
    # Print a few symbols for debugging
    sample = list(exchange.markets.keys())[:10]
    print(f"[DEBUG] Sample available symbols: {sample}")
...
def build_symbol_variants(a, b):
    """Return both slash and no-slash versions."""
    return [f"{b}/{a}", f"{b}{a}"]

def get_symbols_from_triangles():
    symbols = set()
    for a, b, c in TRIANGLES:
        symbols.update(build_symbol_variants(a, b))  # USDT/BTC? Actually we want BTC/USDT
        symbols.update(build_symbol_variants(c, b))  # BTC/ETH
        symbols.update(build_symbol_variants(a, c))  # ETH/USDT
    # But we need the correct base/quote order: for triangle USDT->BTC->ETH->USDT, we need:
    # BTC/USDT, BTC/ETH, ETH/USDT
    # So we'll override with specific pairs.
    # Let's build manually:
    sym_set = set()
    for a, b, c in TRIANGLES:
        sym_set.add(f"{b}/{a}")   # BTC/USDT
        sym_set.add(f"{b}/{c}")   # BTC/ETH
        sym_set.add(f"{c}/{a}")   # ETH/USDT
        # also add no-slash variants
        sym_set.add(f"{b}{a}")    # BTCUSDT
        sym_set.add(f"{b}{c}")    # BTCETH
        sym_set.add(f"{c}{a}")    # ETHUSDT
    return list(sym_set)

ALL_SYMBOLS = get_symbols_from_triangles()
print(f"[DEBUG] ALL_SYMBOLS = {ALL_SYMBOLS}")
...
def load_markets():
    ...
    try:
        exchange.load_markets()
        # Determine which symbol format exists
        found = 0
        for sym in ALL_SYMBOLS:
            if sym in exchange.markets:
                markets[sym] = {
                    'step': exchange.markets[sym]['precision']['amount'],
                    'minNotional': exchange.markets[sym]['limits']['cost']['min']
                }
                found += 1
        print(f"✅ Loaded markets for {found} symbols")
        if found == 0:
            print("[WARN] No symbols matched. Check available symbols in logs.")
    except Exception as e:
        print(f"[FATAL] Could not load markets: {e}")

def round_qty(qty, symbol):
    # Find the correct symbol in markets (try both formats)
...
def get_price(symbol):
    # Try the symbol as is, then alternative format
...
if __name__ == "__main__":
    ...
    print("[BOT] Running on TESTNET – all trades are paper.")
    main_loop()
