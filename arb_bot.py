...
def scan():
    ...
    print(f"[SCAN] Running scan #{scan_count+1}", flush=True)   # <-- this should appear
    reset_daily()
    ...
    if scan_count % 10 == 0:
        if best_tri is not None:
            print(f"[SCAN #{scan_count}] Best: {best_profit:.2f}% from {best_tri[0]}{best_tri[1]}{best_tri[2]}, spread {best_spread:.3%}", flush=True)
        else:
            print(f"[SCAN #{scan_count}] No valid triangle found (best profit {best_profit:.2f}%)", flush=True)
    ...
    if best_tri is not None and best_profit >= PROFIT_THRESHOLD:
        ...
        if scan_count % 5 == 0:
            print(f"[SCAN] No trade – best profit {best_profit:.4f}%, threshold {PROFIT_THRESHOLD:.2f}%", flush=True)
