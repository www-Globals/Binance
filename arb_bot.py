...
# ---------- CONSTANTS (safe for testnet) ----------
KES_RATE = 130
TAKER_FEE = 0.001
PROFIT_THRESHOLD = 0.0035
SLIPPAGE_BUFFER = 0.004
TRADE_FRACTION = 0.05          # 5% of balance per trade (safe)
MAX_TRADE_USD = 5000
MIN_TRADE_USDT = 5
MIN_PROFIT_KES = 20
MAX_SPREAD = 0.002
...
def load_markets():
    ...
    try:
        ...
        print(f"[MARKETS] Loaded {len(VALID_TRIANGLES)} valid triangles")
...
def round_qty(qty, symbol):
    """Round to lot step, ensure it's >= minNotional."""
    if symbol not in markets:
        return max(math.floor(qty / 0.001) * 0.001, 0.001)
    step = markets[symbol]['step']
    minN = markets[symbol]['minNotional']
    rounded = math.floor(qty / step) * step
    if rounded < minN / get_price(symbol):  # ensure value >= minNotional
        rounded = math.ceil((minN / get_price(symbol)) / step) * step
    if rounded <= 0:
        rounded = step
    return rounded
...
def execute_trade(a, b, c, trade_amount_usd, gross_spread, legs):
    ...
    try:
        ...
        # ----- LEG 1: Buy b with a (USDT) – use quoteOrderQty to avoid lot size errors -----
        # For market buy, we can use quoteOrderQty in ccxt by specifying 'cost' parameter? 
        # Actually ccxt doesn't support quoteOrderQty directly for market buys; we must calculate quantity.
        price = p_ab
        if price <= 0:
        ...
        # Ensure quantity meets minNotional
        if base_qty * price < markets[sym1]['minNotional']:
            base_qty = round_qty(markets[sym1]['minNotional'] / price, sym1)
