...
def scan():
    ...
    print(f"[SCAN] Running scan #{scan_count+1}")  # always print scan count
    reset_daily()
    ...
    # ----- SAFELY print the best triangle -----
    if scan_count % 10 == 0:
        if best_tri is not None:
            print(f"[SCAN #{scan_count}] Best: {best_profit:.2f}% from {best_tri[0]}{best_tri[1]}{best_tri[2]}, spread {best_spread:.3%}")
        else:
            print(f"[SCAN #{scan_count}] No valid triangle found (best profit {best_profit:.2f}%)")

    # ----- Trade if opportunity exists -----
    if best_tri is not None and best_profit >= PROFIT_THRESHOLD:
        ...
        # print a short message every few scans to show no trade
        if scan_count % 5 == 0:
            print(f"[SCAN] No trade – best profit {best_profit:.4f}%, threshold {PROFIT_THRESHOLD:.2f}%")
...
if __name__ == "__main__":
    ...
    if VALID_TRIANGLES:
        daily_start_balance = get_total_usdt_balance()
        print(f"[BOT] Starting balance: {daily_start_balance:.2f} USDT")
