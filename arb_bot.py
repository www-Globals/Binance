...
ws_prices = {}            # kept for compatibility (will be overwritten)
        ...
        try:
            ...
            for sym, data in tickers.items():
                if sym in ALL_SYMBOLS:
                    market_prices[sym] = data['last']  # or 'bid'/'ask'?
                    # we'll use 'last' as the mid price for simplicity
            # update ws_prices for compatibility
            global ws_prices
...
def load_markets():
    """Load symbol information (lot size, min notional) from ccxt."""
    global markets
    try:
        # load markets will fetch all market data
        exchange.load_markets()
        for sym in ALL_SYMBOLS:
            if sym in exchange.markets:
                info = exchange.markets[sym]
                # extract lot size and min notional
...
def get_price(symbol):
    """Return latest price from cache."""
...
def execute_trade(a, b, c, trade_amount_usd):
    ...
    try:
        ...
        # LEG 1: Buy b with a (USDT) – using quote order quantity
        sym1 = f"{b}{a}"
        ...
        # market buy using base quantity (CCXT requires amount in base asset)
